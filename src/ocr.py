...
def run_single_ocr(image_path: str, output_txt: str = None, gpu: bool = False, lang: str = 'es'):
...
def run_batch_ocr(input_dir: str, output_csv: str, gpu: bool = False, lang: str = 'es'):
    ...
    for image_path in image_files:
        ...
        if processed is not None:
            ...
            results_data.append({'filename': filename, 'result': detected_text, 'time_seconds': round(elapsed, 3)})
        else:
            results_data.append({'filename': filename, 'result': 'ERROR_LOADING', 'time_seconds': 0})
